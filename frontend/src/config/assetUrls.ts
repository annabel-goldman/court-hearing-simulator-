...
export const USE_S3_ASSETS = rawUseS3Assets === 'true' || rawUseS3Assets === '1' || rawUseS3Assets === 'yes'
...
export function resolveAssetUrl(localAssetUrl: string, assetPath: string): string {
  if (!USE_S3_ASSETS || !rawAssetBaseUrl) {
    return localAssetUrl
  }
  return `${normalizeBaseUrl(rawAssetBaseUrl)}${encodeAssetPath(assetPath)}`
}
